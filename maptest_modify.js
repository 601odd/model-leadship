...
function initMap() {
    ...
    path1 = pathsObject.path1.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path2 = pathsObject.path2.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path3 = pathsObject.path3.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path4 = pathsObject.path4.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path5 = pathsObject.path5.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path6 = pathsObject.path6.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path7 = pathsObject.path7.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path8 = pathsObject.path8.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path9 = pathsObject.path9.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path10 = pathsObject.path10.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path11 = pathsObject.path11.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path12 = pathsObject.path12.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path13 = pathsObject.path13.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path14 = pathsObject.path14.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path15 = pathsObject.path15.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path16 = pathsObject.path16.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path17 = pathsObject.path17.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path18 = pathsObject.path18.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path19 = pathsObject.path19.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path20 = pathsObject.path20.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path21 = pathsObject.path21.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path22 = pathsObject.path22.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path23 = pathsObject.path23.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path24 = pathsObject.path24.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path25 = pathsObject.path25.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path26 = pathsObject.path26.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path27 = pathsObject.path27.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path28 = pathsObject.path28.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path29 = pathsObject.path29.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path30 = pathsObject.path30.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path31 = pathsObject.path31.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path32 = pathsObject.path32.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path33 = pathsObject.path33.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path34 = pathsObject.path34.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path35 = pathsObject.path35.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path36 = pathsObject.path36.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path37 = pathsObject.path37.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path38 = pathsObject.path38.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path39 = pathsObject.path39.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path40 = pathsObject.path40.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path41 = pathsObject.path41.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path42 = pathsObject.path42.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path43 = pathsObject.path43.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path44 = pathsObject.path44.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path45 = pathsObject.path45.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path46 = pathsObject.path46.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path47 = pathsObject.path47.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path48 = pathsObject.path48.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path49 = pathsObject.path49.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path50 = pathsObject.path50.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path51 = pathsObject.path51.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });

    path52 = pathsObject.path52.map(p => {
        return new TMap.LatLng(p[0], p[1]);
    });
...
function animateCamera(startPitch, targetPitch, startRotation, targetRotation, duration) {
